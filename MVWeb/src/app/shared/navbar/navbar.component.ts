...
export class NavbarComponent implements OnInit {
  ...
  loadUserInfo() {
    ...
    this._usuario.getUserInfo(id_usuario).subscribe((resp: any) => {
      ...
      if (resp['image_perfil']) {
        this.imgSrc = resp['image_perfil']
      }
    });
  }
  ...
  async crearFBuser(authresp: any, user: any) {
    ...
    this._usuario.loginUsuarioFB(formData)
      .subscribe((resp: any) => {
        this._navbar.recarga_Username('actualizar');
        console.log(resp);
        const token = JSON.stringify(resp['access']).slice(1, -1);
        const refresh = JSON.stringify(resp['refresh']).slice(1, -1);
  ...
  scrollToPaquetes() {
    console.log('paquete');
    this.router.navigate(['/home/inicio/'], { fragment: 'paquete' });
}
}
